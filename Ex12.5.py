...
#    Actual problem: Start at: http://py4e-data.dr-chuck.net/known_by_Cecily.html
...
from bs4 import BeautifulSoup as gf
import ssl
...
position=int(positio)-1
o=int(input('Enter count : '))
html = urllib.request.urlopen(url,context=ctx).read()
soup = gf(html,'html.parser')
tags = soup('a')

j=[]
for tag in range(o):
    links=tags[position].get('href',None)
    print('Retrieving : ',links)
    j.append(tags[position].contents[0])
